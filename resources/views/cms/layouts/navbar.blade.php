            ...
            <div tabindex="0" class="mt-3 z-[1] bg-[#1D2A47] -left-[30] card card-compact  dropdown-content w-80 shadow">
                ...
                @foreach ($notifs as $item)
                    <a class="text-white gap-4 flex px-2 py-1 cursor-pointer border-b border-blue-950 {{ $item->read_at == null ? 'bg-[#384478]' : 'bg-[#1D2A47]'  }}" href="{{ route('notif.read', $item->id) }}">
                        <img src="{{ strpos($item->userFrom->image_user, 'http') !== false ? $item->userFrom->image_user : url('storage/profile_picture/'.$item->userFrom->image_user) }}" onerror="this.src='{{ asset('assets/images/default-profile.png') }}'" alt="profile" class="w-10 h-10 rounded-full">
