    ...
    <div id="tab-contents" class="w-full lg:w-3/4 2xl:w-5/6 our-regulation-background border rounded-xl border-blue-950 ">
        ...
        @foreach ($knowledges as $index => $know)
            <div class="w-full flex flex-col px-4 py-5 text-white" id="tab{{$index}}">
                ...
                <div class="mt-6">
                    <a href="{{ $know->file_path }}" target="_blank" class="text-white px-4 w-6/12 2xl:w-3/12text-sm py-2 rounded-2xl flex items-center login-button bg-login-button">
                        ...
                        <div class="flex flex-col text-left text-xs">
                            <p>Download {{ $know->judul }}</p>
                            <p class="font-extralight">{{ $know->file_size }} MB</p>
                        </div>
                    </a>
                </div>
